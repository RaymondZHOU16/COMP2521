...
struct route advancedNavigate(RoadMap map, int fromNode, int toNode,
    ...
    for (int i = route.numLegs - 1; i >= 0; i--) {
        ...
        if (RoadMapHasTrafficLights(map, path[indexPath]) &&
            path[indexPath - 1] != toNode) {
            currTime = TimeAddMinutes(currTime, -TRAFFIC_LIGHT_WAIT);
        }
...
// add minutes to time, if minutes > 60, add to hour, elif mintues < 0, subtract
struct time TimeAddMinutes(struct time t, int minutes) {
    struct time newTime = t;
    ...
    if (newTime.minute >= MINUTES_PER_HOUR) {
        ...
        newTime.hour += newTime.minute / MINUTES_PER_HOUR - 1;
        newTime.minute = MINUTES_PER_HOUR + newTime.minute % MINUTES_PER_HOUR;
    }
    // check if hour is over 24
    if (newTime.hour >= 24) {
        newTime.hour %= 24;
