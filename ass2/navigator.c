...
struct route advancedNavigate(RoadMap map, int fromNode, int toNode,
    ...
    for (int i = 0; i < numNodes; i++) {
        dist[i] = (struct time){.hour = 99, .minute = 99}; // Assuming 24:60 as infinity
        prev[i] = -1;
        visited[i] = false;
    }
    ...
    for (int count = 0; count < numNodes - 1 && u != -1; count++) {
		...
		for (int i = 0; i < numFromRoads; i++) {
			...
			if (RoadMapIsClosed(map, fromRoads[i], travelTime)) {
				...
				travelTime = TimeAddMinutes(travelTime, fromRoads[i].travelMinutes);
			}
	...
	route.totalDuration = dist[toNode].hour * MINUTES_PER_HOUR + dist[toNode].minute;
...
static bool findShortestPath(RoadMap map, int fromNode, int toNode, int *dist, int *prev, bool *visited) {
	dist[fromNode] = 0;
    prev[fromNode] = fromNode;
...
// task 3 get route
static void getRoute(RoadMap map, int fromNode, int toNode, struct time time, int *dist, int *prev, bool *visited, struct route *route) {
	// store and reverse tha path
	int *path = malloc(sizeof(int) * RoadMapNumNodes(map));
	if (path == NULL) {
		fprintf(stderr, "error: out of memory\n");
