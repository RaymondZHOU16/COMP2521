...
void listInsertOrderedHelper(struct node **ptr, int value) {
    if (*ptr == NULL || value <= (*ptr)->value) {
        struct node *tmpNode = newNode(value);
        tmpNode->next = *ptr;
        *ptr = tmpNode;
    } else {
        listInsertOrderedHelper(&((*ptr)->next), value);
    }
}

void listInsertOrdered(struct list *list, int value) {
    listInsertOrderedHelper(&(list->head), value);
}
