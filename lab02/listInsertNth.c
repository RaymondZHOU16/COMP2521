...
void listInsertNthHelper(struct node **ptr, int n, int value) {
    // Base case: if n is 0 or we've reached the end of the list
    if (n == 0 || *ptr == NULL) {
        struct node *new_node = newNode(value);
        new_node->next = *ptr;
        *ptr = new_node;
    } else { 
    	listInsertNthHelper(&((*ptr)->next), n - 1, value);
	}
}

void listInsertNth(struct list *list, int n, int value) {
    listInsertNthHelper(&(list->head), n, value);
}
